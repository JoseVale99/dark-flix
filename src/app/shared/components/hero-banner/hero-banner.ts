      ...
      <div class="relative z-20 flex flex-col md:flex-row items-center justify-between w-full max-w-7xl mx-auto pt-10 md:pt-20 gap-8 md:gap-4">
        ...
        <div class="w-full md:w-1/2 flex flex-col items-center md:items-start text-center md:text-left">
          ...
          <div class="flex flex-wrap justify-center md:justify-start gap-4 w-full">
            <button [routerLink]="['/movie', featuredPost()?._id]" class="bg-[#e50914] hover:bg-red-700 text-white font-bold py-3 md:py-4 px-8 md:px-12 rounded flex items-center justify-center gap-3 transition-transform hover:scale-105 active:scale-95 shadow-[0_0_20px_rgba(229,9,20,0.4)] text-lg">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6"><path d="M5 3l14 9-14 9V3z"/></svg>
              REPRODUCIR
            </button>
            <button [routerLink]="['/movie', featuredPost()?._id]" class="bg-gray-500/30 hover:bg-gray-500/50 backdrop-blur-md border border-white/10 text-white font-bold py-3 md:py-4 px-8 md:px-10 rounded transition-colors flex justify-center items-center text-lg gap-2">
