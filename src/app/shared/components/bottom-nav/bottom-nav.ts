...
@Component({
  ...
  template: `
    <!-- Menú Desplegable (Bottom Sheet) para Categorías -->
    @if (isCategoriesOpen()) {
      ...
      <div class="fixed bottom-24 left-4 right-4 z-50 bg-[#141414]/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-[0_10px_50px_rgba(0,0,0,0.9)] p-2 flex flex-col animate-fade-in origin-bottom md:hidden">
        ...
        <h3 class="text-[10px] font-black text-gray-400 uppercase tracking-widest px-4 mb-2 text-center">Colecciones</h3>
        ...
        <a routerLink="/series" routerLinkActive="bg-white/10 text-[#e50914]" (click)="toggleCategories()" class="text-white font-bold text-base md:text-lg p-4 hover:bg-white/10 rounded-xl transition-colors flex items-center gap-4">
          <div class="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center shrink-0 shadow-inner group-hover:border-white/30 transition-colors">
             <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="w-5 h-5 text-gray-300"><rect x="2" y="7" width="20" height="15" rx="2" ry="2" /><polyline points="17 2 12 7 7 2" /></svg>
          </div>
          Series
        </a>
        <a routerLink="/movies" routerLinkActive="bg-white/10 text-[#e50914]" (click)="toggleCategories()" class="text-white font-bold text-base md:text-lg p-4 hover:bg-white/10 rounded-xl transition-colors flex items-center gap-4">
           <div class="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center shrink-0 shadow-inner group-hover:border-white/30 transition-colors">
             <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="w-5 h-5 text-gray-300"><rect x="2" y="2" width="20" height="20" rx="2.18" ry="2.18" /><line x1="7" y1="2" x2="7" y2="22" /><line x1="17" y1="2" x2="17" y2="22" /><line x1="2" y1="12" x2="22" y2="12" /><line x1="2" y1="7" x2="7" y2="7" /><line x1="2" y1="17" x2="7" y2="17" /><line x1="17" y1="17" x2="22" y2="17" /><line x1="17" y1="7" x2="22" y2="7" /></svg>
          </div>
          Películas
        </a>
        <a routerLink="/animes" routerLinkActive="bg-white/10 text-[#e50914]" (click)="toggleCategories()" class="text-white font-bold text-base md:text-lg p-4 hover:bg-white/10 rounded-xl transition-colors flex items-center gap-4">
           <div class="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center shrink-0 shadow-inner group-hover:border-white/30 transition-colors">
             <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="w-5 h-5 text-gray-300"><path stroke-linecap="round" stroke-linejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
          </div>
          Animes
        </a>
      </div>
    ...
    <nav class="fixed bottom-0 left-0 right-0 z-50 bg-[#0a0a0a]/95 backdrop-blur-xl border-t border-white/5 shadow-[0_-10px_40px_rgba(0,0,0,0.8)] pt-3 pb-6 px-2 md:hidden">
      <!-- MÁXIMO 4 ÍTEMS, limpios y espaciados -->
      <ul class="flex justify-between items-center text-[10px] font-bold text-gray-500 uppercase tracking-wider w-full">
        
        <!-- Tab 1: Inicio -->
        <li class="flex-1">
          <a routerLink="/" routerLinkActive="text-[#e50914] scale-110 drop-shadow-[0_0_8px_rgba(229,9,20,0.8)]" [routerLinkActiveOptions]="{exact: true}" class="flex flex-col items-center gap-1 transition-all outline-none focus:outline-none [-webkit-tap-highlight-color:transparent]">
        ...
        <!-- Tab 2: Buscar -->
        <li class="flex-1">
          <a routerLink="/categories" routerLinkActive="text-[#e50914] scale-110 drop-shadow-[0_0_8px_rgba(229,9,20,0.8)]" class="flex flex-col items-center gap-1 transition-all outline-none focus:outline-none [-webkit-tap-highlight-color:transparent]">
        ...
        <li class="flex-1">
          <button (click)="toggleCategories()" 
                  ...
                  class="flex flex-col items-center gap-1 transition-all outline-none focus:outline-none [-webkit-tap-highlight-color:transparent] hover:text-white cursor-pointer w-full">
        ...
        <!-- Tab 4: Mi Lista -->
        <li class="flex-1">
          <a routerLink="/mi-lista" routerLinkActive="text-[#e50914] scale-110 drop-shadow-[0_0_8px_rgba(229,9,20,0.8)]" class="flex flex-col items-center gap-1 transition-all outline-none focus:outline-none [-webkit-tap-highlight-color:transparent]">
            ...
            <span class="text-[10px]">Mi Lista</span>
          </a>
        </li>
